...
def score(
    ...
    try:
        # Import the scoring code only when the user actually submits a
        # response. This keeps Render startup fast enough for port detection.
        ...
        # If Hugging Face, torch, sentence-transformers, or model loading fails,
        # return a JSON error instead of crashing the whole FastAPI process.
        return JSONResponse(
            {
                "error": (
                    "The OCSAI-lite model could not be loaded or used. "
                    "Please try again after the app finishes warming up."
                ),
                "detail": str(exc),
            },
            status_code=503,
